	...
	<div class="container-fluid">
		<div class="row">
			<div class="navbar navbar-fixed-top">
				<div class="navbar-inner">
					<ul class="nav">
						...
						<li class="active"><a href="getAllEquipment">设备管理</a></li>
			...
			<%
				...
				if(context.get("equipList")!=null){
					equipList=(List<Equipment>)context.get("equipList");
			}
		 	%>
				<section id="equips">
					...
					<div class="panel-body">
						<table class="table">
							...
							<tbody>
								...
								%>
								<tr>
									<td><%=equipList.get(i).getName()%></td>
									<td>
										<button class="btn btn-mini btn-info"
											onclick="equip_click(<%=equipList.get(i).getId()%>)">
											编辑</button>
									</td>
									<td>
										<button class="btn btn-mini btn-danger"
											onclick="equip_delete(<%=equipList.get(i).getId()%>)">
											删除</button>
									</td>
								</tr>
								<%
					...
					<div class="panel-body">
						<table class="table">
							<tr>
								...
								<td><textarea rows="1" disabled="disabled" id="equipID"
										name="id"></textarea>
								<td>
							</tr>
							<tr>
								...
								<input style="display: none;" id="picUrl" 
									<%
										if (context.get("equipPicUrl") != null) { 
									%> 
										 value="<%=context.get("equipPicUrl")%>"
									<%
 										} 
 									%> 
 									> 
        							...
        							<input type="button" value="上传" onclick="ajaxFileUpload();"><hr>
 									<!-- 	<a class="btn btn-info" target="_blank" href="uploadEquipPic.jsp" >添加图片</a> -->
 									<img id="equipPic" width="150px" alt="" src="<%=context.get("equipPicUrl")%>">
								</td>
							</tr>
							<tr>
								<td>设备名：
							</td>
								<td><textarea rows="1" id="equipName" ></textarea>
								<td>
							</tr>
							<tr>
								<td>设备说明：
							...
							<tr>
								<td></td>
								<td colspan="3">
									<button class="btn btn-info" id="btnCommit">提交</button></td>
							</tr>
						</table>
					</div>
				</div>
...
<script type="text/javascript"> 
  function ajaxFileUpload(){
        $.ajaxFileUpload({
           	url:'uploadEquipPic',//用于文件上传的服务器端请求地址
            ...
            success: function (data, status)  //服务器成功响应处理函数
            {
             	//alert(data.message);
             	alert("图片保存成功！");
             	$("#equipPic").attr("src",data.message);
...
<script type="text/javascript">
/* 点击显示信息 */
function equip_click(id){
	$("#equipID").val(id);
	$.ajax({
		type : "POST",//方法
		url : "getEquipInfoJson",//链接
		dataType : "json",
		data : {
			equipId : id,
		},
		success : function(data) {
			$("#equipPic").attr("src",data.result.pic_url);
			$("#equipName").val(data.result.name);
			$("#editor").html(data.result.intro);
		}
	}); 
}
/* 删除 */
function equip_delete(id){
	if(window.confirm("你确定要删除该设备吗？")){
		location="deleteEquip?equipId="+id+"";
	}
	else{
		 return false;
	}
...
$("#btnCommit").click(function(){
	...
	$.ajax({
		...
		success : function(result) {
			...
			window.location.href="getAllEquipment";
		},
