		...
		<div class="container">
			<div class="row">
				...
				<div class="main col-md-9">
					...
					<!-- 团体查询 (默认不显示)-->
					<div id="panelTeam" style="display: none"
							...
							<div class="panel-body">
								<!-- <form action="getUnitEmpInfo" method="post"> -->
									<div id="formUnitID" class="form-group">
										<label>单位编号</label> <input type="text" class="form-control"
											id="unitID" name="unitID" placeholder="单位编号">
									</div>
									<div id="formUnitPassword" class="form-group">
										<label>查询密码</label> <input type="password"
											class="form-control" id="unitPassword" name="unitPassword"
											placeholder="查询密码">
									</div>
									<div class="alert alert-danger" id="unitValidate"
										style="display: none">
										<strong id="unitValidateTip">Oh snap!</strong>
									</div>
									<button id="btnTeamCheck" type="button"
										class="btn radius btn-default">查询</button>
								<!-- </form> -->
							</div>
						</div>
				...
				$.ajax({
					...
					success : function(data) {
						if (data.result.status == 200) {
							window.location.href="getPhyicalList?IDCard="+IDCard+"";
							validateFlag = true;
							return true;
						} else {
							$("#validate").css("display", "block");
				...
				$.ajax({
					...
					success : function(data) {
						if (data.result.status == 2) {
							window.location.href="getUnitEmpInfo?unitID="+unitID+"";
							validateFlag = true;
							return true;
						} else {
							$("#formUnitPassword").attr("class",
									"form-group has-error");
							$("#unitValidate").css("display", "block");
